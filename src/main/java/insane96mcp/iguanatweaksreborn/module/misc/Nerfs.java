...
public class Nerfs extends Feature {
	static final String LAST_FISHING_POS_TAG = InsaneSurvivalOverhaul.RESOURCE_PREFIX + "last_fishing_pos";
	static final String LAST_FISHING_COUNT_TAG = InsaneSurvivalOverhaul.RESOURCE_PREFIX + "last_fishing_count";
	@Config
		...
		if (persistentData.contains(LAST_FISHING_POS_TAG)) {
			...
			if (distance <= 6) {
				...
				if (lastFishingCount >= 10) {
					...
					event.getHookEntity().getPlayerOwner().displayClientMessage(Component.translatable(InsaneSurvivalOverhaul.MOD_ID + ".too_much_fishing_in_this_spot"), true);
				}
